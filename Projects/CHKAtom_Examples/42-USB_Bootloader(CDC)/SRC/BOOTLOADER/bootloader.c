...
static void ProcessTransDataMsg(MessageType_t* pMsg)
{
    ...
    if((M_Control.currentPkgNo == (pDataFrame->currentPkgNo-1)) || (M_Control.currentPkgNo == pDataFrame->currentPkgNo))
    {
        ...
        if(needWrite)
        {
            Fn_EreaseFlash(M_Control.write_addr, 1);
            ...
            else if(Fn_WriteFlash(M_Control.write_addr, (uint32_t*)pDataFrame->content, FLASH_PAGE_SIZE) == OP_SUCCESS)
            {
...
static void ProcessAppVerificationMsg(MessageType_t* pMsg)
{
    ...
    if(M_Control.currentPkgNo == M_Control.total_pkg)
    {
        ...
        {
            if(Fn_WriteFlash(APP_START_ADDR, (uint32_t*)CacheFirstPg, FLASH_PAGE_SIZE) == OP_SUCCESS)
            {
                rspFrame.status = RCV_OK;
            }
        }
...
static void ProccessAppCheckMsg(MessageType_t* pMsg)
{
    ...
    if(Fn_IsAppValid())
    {
        for(volatile uint32_t i=0; i<26000; i++);
				...
				PORTC->PCR[LED0_PIN] = PORT_PCR_MUX(0);  //DeInit LED Pin
...
{
    Flash_Init();
    SysTick_Cfg(CPUInfo.CoreClock/10);
	  //UART_Init(uartch,buadrate);
	 // pUARTx = uartch;
    fn_queue_init();
...
{
	u16 i,RecCnt;
		RecCnt = USB_CDCComRx(&UARTRxBuf);
		if(RecCnt != 0)
		{
			for(i=0;i<RecCnt;i++)
			{
				Fn_RxProcData(UARTRxBuf[i]);
			}
		}
    if (fn_msg_exist())
    {
