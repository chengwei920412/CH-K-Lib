...
#if (SYSTEM_SUPPORT_OS == 1) 	//如果使用UCOS兼容模式
#include "includes.h"					//ucos 使用	 
u32 OS_ReloadValue;         //SysTick重装值
/***********************************************************************************************
 功能：SysTick内核中断
 形参：0
 返回：0
...
#if (SYSTEM_SUPPORT_OS == 1) 					//如果OS_CRITICAL_METHOD定义了,说明使用ucosII了.
	SysTick->CTRL|=1<<1;   						//开启SYSTICK中断
	SysTick->LOAD=PInfo.CoreClock/OS_TICKS_PER_SEC; 	//每1/OS_TICKS_PER_SEC秒中断一次	
...
void DelayMs(u32 ms)    
{
	...
	if(ms != 0) DelayUs((u32)(ms*1000));	//普通方式延时 	
}
