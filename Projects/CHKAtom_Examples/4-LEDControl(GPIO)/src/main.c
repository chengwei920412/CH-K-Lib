...
*   3: kLED1,kLED2是在led_chkatom.h中定义的枚举量 前面加k 只是为了表示他们是枚举常量
*   4: LED_PinLookup_chkatom[] 是一个保存在 led_chkatom.c中的 结构体数组 存放板子的LED配置信息 chkatom 是超核K60核心板的简写
*   5: kNumOfLED 是存放在 led_chkatom.h中的枚举常量 表示板子LED的总数 在CHK60开发板中 它就是2
*   6: LED_Init 第一个参数传入 LED_PinLookup_chkatom 这个结构体指针 第二个参数传入 结构体成员数量 也就是板子上LED的数目
